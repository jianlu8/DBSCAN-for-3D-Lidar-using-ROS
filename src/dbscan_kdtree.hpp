...
public:
    ...
    void extract(std::vector<pcl::PointIndices>& cluster_indices) {
        ...
        for (int i = 0; i < input_cloud_->points.size(); i++) {
            ...
            double p_x = input_cloud_->points[i].x;
            double p_y = input_cloud_->points[i].y;
            double p_z = input_cloud_->points[i].z;
            // //my promote

            double alpha0 = 2.0 / 180 * M_PI;
            double beta_i = atan(abs(p_z) / sqrt(p_x * p_x + p_y * p_y));
            double r_i = sqrt(p_x * p_x + p_y * p_y + p_z * p_z);
            double eps_i=sin(alpha0)/sin(beta_i-alpha0)*r_i;
            eps_i=eps_;


            // ROS_INFO("alpha0= %f, beta_i=%f, eps_i=%f, r_i=%f",alpha0, beta_i, eps_i, r_i);
        
            int nn_size = radiusSearch(i, eps_i, nn_indices, nn_distances);
            if (nn_size < minPts_) {
                is_noise[i] = true;
                continue;
            }
            ...
            while (sq_idx < seed_queue.size()) {
                ...
                nn_size = radiusSearch(cloud_index, eps_i, nn_indices, nn_distances);
                if (nn_size >= minPts_) {
                    for (int j = 0; j < nn_size; j++) {
